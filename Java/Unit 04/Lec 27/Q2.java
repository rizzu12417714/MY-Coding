...
Anna is creating a user ID management system that handles a list of user IDs for a given number of entries.
  The system allows users to view and update a specific user ID based on its index in the list. The program needs to manage various types of errors:



Validates List Size: The number of user IDs must be between 1 and 500. If the size is out of this range, it throws an IllegalArgumentException
with: "Error: Number of user IDs must be between 1 and 500".
Handles Invalid Index: When accessing or updating a user ID, if the index is out of bounds, it throws
  an ArrayIndexOutOfBoundsException with: "Error: Invalid index. Please select a valid index from the list".
Handles Non-Numeric Inputs: If any non-integer value is entered for user IDs or index, it catches a
  NumberFormatException with: "Error: Please enter valid integer values for user IDs".
...
The next line contains an integer i, representing the position in the array for which the user ID should be accessed and updated.
...
// Utility Class
class UserIDManagerUtil {

    public static void processUserIDs(Scanner scanner) {

        try {
            // Read number of user IDs
            ...
            // Read user IDs
            for (int i = 0; i < n; i++) {
                userIDs[i] = Integer.parseInt(scanner.nextLine().trim());
            }
...
class UserIDManager {
    ...
    public static void main(String[] args) {
        ...
        UserIDManagerUtil.processUserIDs(scanner);
